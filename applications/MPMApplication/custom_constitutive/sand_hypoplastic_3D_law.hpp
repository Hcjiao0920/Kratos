...
// SandHypoplastic3DLaw: Kratos ConstitutiveLaw wrapper around the
// standalone sand-hypoplasticity kernel (sand_hypoplastic_kernel.hpp).
// Effective-Cauchy in / out at the kernel boundary; this wrapper handles
// the Voigt slot 4<->5 swap, total<->effective pore-pressure conversion,
// void-ratio first-call init (Bauer 1996), the bulk_w*1\otimes1 tangent
// correction, and failure_class -> KRATOS_ERROR / pnewdt routing.
//
// References of the form .for:NNN throughout this law point into the
// reference Fortran UMAT implementation of this model (umat_hypoplasticity
// by A. Niemunis, publicly distributed via soilmodels.info); they document
// algorithm provenance.

#if !defined (KRATOS_SAND_HYPOPLASTIC_3D_LAW_H_INCLUDED)
#define       KRATOS_SAND_HYPOPLASTIC_3D_LAW_H_INCLUDED
...
// Hypoplastic constitutive law for granular materials (sand) after
// von Wolffersdorff (1996), extended with the intergranular strain
// concept of Niemunis & Herle (1997). Time integration uses adaptive
// error-controlled RKF-2(3) substepping following Fellin & Ostermann.
//
// References:
//  - P.-A. von Wolffersdorff (1996), "A hypoplastic relation for granular
//    materials with a predefined limit state surface",
//    Mechanics of Cohesive-frictional Materials 1(3), 251-271.
//  - A. Niemunis, I. Herle (1997), "Hypoplastic model for cohesionless
//    soils with elastic strain range",
//    Mechanics of Cohesive-frictional Materials 2(4), 279-299.
//  - E. Bauer (1996), "Calibration of a comprehensive hypoplastic model
//    for granular materials", Soils and Foundations 36(1), 13-26.
//  - W. Fellin, A. Ostermann (2002), "Consistent tangent operators for
//    constitutive rate equations",
//    Int. J. Numer. Anal. Meth. Geomech. 26, 1213-1233.
class KRATOS_API(MPM_APPLICATION) SandHypoplastic3DLaw
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(SandHypoplastic3DLaw);

    SandHypoplastic3DLaw();
    SandHypoplastic3DLaw(const SandHypoplastic3DLaw& rOther);
    ~SandHypoplastic3DLaw() override = default;
    ...
    // MPMUpdatedLagrangian::Check: implicit runs require
    // Deformation_Gradient; explicit runs accept EITHER
    // Deformation_Gradient OR Velocity_Gradient. This law consumes
    ...
    // and differentiates against the persisted previous-step strain
    // `mEpsPrevExplicitKratos` to get the kernel-driving Δε. The explicit
    // branch must NOT use F-based polar decomposition: the F handed in on
    // the explicit path carries no spin, so that route would silently
    // freeze rotation.
    ...
    // all committed history and leaves the law in a state equivalent to
...
private:
    ...
    // Hughes-Winget corotational bookkeeping.
    ...
    // contract puts objectivity inside the law (MPMUpdatedLagrangian
    // passes the stress in/out unchanged).
    //
    // On the EXPLICIT path mF0_3x3 is NOT used for kinematics
    ...
    // Begin-of-step strain in Kratos Voigt order for the EXPLICIT branch.
    // The explicit kernel-driving increment is
    ...
    // The implicit branch does not touch this member.
    //
    ...
    // members. Serialized for restart symmetry with the other committed
    // history members.
    ...
    // (MPM passes FT = F * F0 via SetDeformationGradientF).
    ...
    // does NOT populate rVariables.StrainVector itself, so this law owns
    // computing it.
    //
    ...
    // intergranular state fed to the kernel: mixing log(U) with R-rotated
    // σ/δ silently corrupts the constitutive update under combined
    // rotation+stretch.
    ...
    // law's path-dependent state -- it cannot be reconstructed from
    // materials JSON + INITIAL_STRESS_VECTOR alone once loading has
    // begun, because that only captures the initial-configuration
    // snapshot. Order is fixed by the tag strings -- do not reorder
    // save/load entries asymmetrically.
...
#endif // KRATOS_SAND_HYPOPLASTIC_3D_LAW_H_INCLUDED
