...
// Kernel implementation. See sand_hypoplastic_kernel.hpp for the
// contracts, the Voigt convention, and the provenance of the
// `00_hypo_is_original.for` / `.for:NNN` references (they point into
// the reference Fortran UMAT implementation of this model,
// umat_hypoplasticity by A. Niemunis, distributed via soilmodels.info).
//
...
#include "sand_hypoplastic_kernel.hpp"
...
CheckParmsResult check_parms(const Props16& props) {
    ...
    // Fortran reference would silently NaN-poison when r_uc = 0;
    // get_tan already rejects it as a defensive deviation, but
    ...
    //     through validated parms (kept as defense-in-depth).
...
TangentResult get_tan(const double* deps,
    ...
    // comment in the original; this port honours the post-Tamagnini
    // umat design in which the caller short-circuits via inittension
    ...
    if (fb_temp1 < 0.0) {
        ...
        // The STOP is translated into an error code; M/L/N/H stay zero
    ...
    // =====================================================================
    if (istrain == 1) {
        // Defensive guard: structurally unreachable through validated
        // parms, because `check_parms` enforces "r_uc > 0 if
        // m_R > 0.5". Kept here as defense-in-depth in case a caller
        // constructs a Parms16 that bypasses check_parms (e.g. via
        // memcpy or a future test seam) -- it preserves the structural
        // invariant that the IS branch never divides by zero.
        //
        // Rationale (deviation from Fortran): the Fortran reference
        // computes `rho = norm_del / r_uc` unconditionally; with the
        // common zero-IS initial state and an accepted r_uc == 0 (or
        ...
        // of the routine. Mirrored here with explicit zero-init.
...
CheckRkfResult check_RKF(const double* y, const Parms16& parms) {
    ...
    // Deviation from the Fortran reference: the admissibility check
    // operates on RAW sig (not sig_star). p_t (parms[1]) remains a
    // parameter used elsewhere in the kernel (eta = sig_star/I1 in
    // get_tan / rhs) for the model's own regularization, but not in
    // the admissibility threshold. See the long-form rationale below
    // this block. parms is otherwise unused at this point.
    (void)parms;
    ...
    // This port's stricter "fail closed on corruption" behavior is the
    ...
    // Deviation from the Fortran reference: the tensile check runs on
    // RAW sig, not on sig_star.
    //
    ...
    //
    // Empirical failure mode (observed in an MPM sand-column collapse
    // benchmark): a corner material point at the column top (free
    // face, no overburden) drifted to raw sig = [+0.62, +0.03, +0.11, -0.30,
    ...
    //
    // The fix: compute pmean and principal_stresses_3 from raw `sig`,
    ...
    // min_principal <= 0 and the "tension" label fires.
...
NormResResult norm_res(const double* y_til, const double* y_hat) {
    ...
    // Letting either Inf or NaN escape would feed the RKF driver's
    // step-size control with garbage. Force the sentinel.
...
Rkf23Result rkf23_update(const double* y_init,
    ...
    // calls. This kernel API is exposed independently, so naive
    ...
    // maxnint fires; this port rejects explicitly with error = 3. Also
    ...
    // Same family of failure as the err_tol gap above; caught
    // preemptively here so the input-contract surface is symmetric.
    ...
    // every check_RKF gate inside the substep loop. This closes the
    // rkf23-internal corruption gap so `integrate_step` does not have
    // to demote everything to RkfReject.
    ...
    while (T_k < 1.0) {
        ...
        // 11b. Poisoned-residual fail-closed. norm_res sets a
        //      corruption sentinel
        ...
        //      silently committed. This port respects the explicit
...
StepResult integrate_step(const double* sig_n,
    ...
    // it would deadlock the caller: it cuts back, reads zero from
    // statev again, and never makes progress. NaN/Inf are still hard
    // rejects -- they cannot be normalized and almost certainly
    // indicate a real fault upstream.
    if (!std::isfinite(dtime)   || dtime   <= 0.0
     ...
     || maxnint <= 0) {
        ...
        // immediately on the first ksubst > maxnint check
        // and integrate_step would classify it as RkfReject -- a
        // caller that retried with cutback would loop forever.
        r.error          = 3;
        r.failure_class  = FailureClass::ContractViolation;
    ...
    // the elastic short-path bypasses those gates, so the gap must
    // be closed here.
    if (!input_corrupt) {
        for (int i = 0; i < 6; ++i) {
    ...
    // this gap open would contradict that.
    if (!input_corrupt) {
        const double norm_D2 = dot_vect(DotProductKind::StrainLike,
                                        deps_np1, deps_np1, 6);
        const double norm_D  = std::sqrt(norm_D2);
    ...
    if (input_corrupt) {
        ...
        // inputs propagate back unchanged -- no sanitizing). r.D
    ...
    if (chk.error != 0) {
        ...
        // tangent; mirrored here by the shared get_tan call below.
        ...
        if (zero_deps) {
            ...
            if (rkf.error == kErrorOk) {
                ...
                // rkf23_update sets failure_class precisely at every
                ...
                // Pass it through verbatim. No demotion to
        ...
        // its caller, so this port would diverge from the reference
        // implementation here.
        //
        ...
        if (r.error == kErrorOk) {
            ...
            if (tan_final.error != kErrorOk) {
                ...
                // rollback shares this output contract.
    ...
    // ---- Low-confinement stress guard on the success output -----------
    // Sand hypoplasticity is rate-form with no explicit yield surface,
    // so the RKF integrator can produce small-but-nonzero deviatoric
    // stress in regions where pmean -> 0 (free-surface MPM particles).
    // Those residual deviators can have q/p well above the
    ...
    // shear strength. When such material points feed their stress into
    // an explicit MPM solve, the spurious internal forces accumulate at
    // low-mass background-grid nodes, destabilize the free surface, and
    // can crash the analysis.
    //
    // Return-mapping plasticity (e.g. HenckyMCPlastic3DLaw) avoids this
    // by construction: the radial return projects every step onto the
    // yield envelope, so q exactly tracks q_max at all p. The same
    // effect is mimicked here as a post-integration scrub. Well-confined
    // bulk material points sit below the envelope and are unaffected;
    // the cap only intervenes where the deviator is already physically
    // meaningless noise.
    if (r.error == kErrorOk) {
        const double p_out = -(r.sig[0] + r.sig[1] + r.sig[2]) / 3.0;
        // Two-tier guard. Below the threshold pressure the state is
        // treated as effectively unconfined and the full stress is
        // zeroed; above it, only the deviator is capped at the
        // Drucker-Prager envelope (else-branch below).
        constexpr double kFreeSurfaceCapPressure = 1.0;  // kPa
        if (p_out < kFreeSurfaceCapPressure) {
            // Zero sigma AND substitute a clean linear-elastic tangent.
            // Zeroing only r.sig would leave r.D as whatever the
            // rate-form integrator produced at the ill-conditioned
            // p -> 0 state. Explicit MPM does not care (only sig enters
            // the internal-force vector), but implicit Newton does: a
            // spurious tangent at free-surface material points
            // contaminates the global stiffness matrix and can make
            // the sparse linear solver fail.
            //
            // Cure: emit the same E=100 kPa, nu=0.48 linear-elastic
            // tangent that the inittension fallback already uses.
            // Physically: at zero confinement the material responds
            // elastically to incoming strain until p climbs back above
            // the threshold, then the regular hypoplastic kernel takes
            // over. Mathematically: this keeps the 6x6 D block
            // contributed to the global stiffness well-conditioned
            // (SPD with bounded condition number).
            //
            // Sigma stays at zero -- sand at p = 0 has zero shear
            // strength.
            ...
            // converts the raw degrees input in-place.
