...
// Standalone C++ port of the sand hypoplasticity model of
// von Wolffersdorff (1996) with the intergranular-strain extension of
// Niemunis & Herle (1997) (limit void ratios after Bauer 1996; adaptive
// error-controlled integration in the style of Fellin & Ostermann).
//
// The port is translated line-by-line from the reference Fortran UMAT
// implementation of this model (umat_hypoplasticity by A. Niemunis,
// publicly distributed via soilmodels.info), referred to below as
// `00_hypo_is_original.for`. References of the form `.for:NNN`
// throughout the kernel point into that Fortran source.
//
// This header declares the kernel namespace `Kratos::SandHypoCpp`.
// Each translated routine mirrors a Fortran subroutine of the same
// root name (`check_parms_h` -> `check_parms`, `dot_vect_h` ->
// `dot_vect`, `matmul_h` -> `matmul`).
//
// =====================================================================
...
// Fortran slots 5 and 6). The wrapping constitutive law
// (`SandHypoplastic3DLaw`) is responsible for swapping
// C++ indices 4 <-> 5 on:
...
//   - the intergranular-strain state whenever it is exposed in
//     Kratos order.
//
...
//
// Runtime persistence rule (binding on the wrapping law): the persisted
// statev MUST be stored in kernel/Abaqus order between calls. A wrapper
// that stored statev in Kratos order would have to swap q[4] <-> q[5]
...
// CalculateValue / SetValue overrides for IS-related variables,
// MaterialResponse setters/getters), never on the persisted
// statev itself. The kernel's Abaqus order is the storage canonical.

#if !defined (KRATOS_SAND_HYPOPLASTIC_KERNEL_H_INCLUDED)
#define       KRATOS_SAND_HYPOPLASTIC_KERNEL_H_INCLUDED

#include <array>
#include <cstddef>

// Only Kratos dependency of this kernel: the DLL export/import macro,
// needed so the C++ test target (which links KratosMPMCore) can call
// the kernel entry points directly. kratos_export_api.h is macro-only.
...
// wrapping law's `Check()`) MUST guarantee finiteness before
// passing `props` here.
KRATOS_API(MPM_APPLICATION) CheckParmsResult check_parms(const Props16& props);
...
KRATOS_API(MPM_APPLICATION) double dot_vect(DotProductKind kind,
                const double* a,
                const double* b,
                std::size_t n);
...
//
// This is the literal port of `matmul_h` (.for:1233-1253). Raw-pointer
// flat arrays are kept deliberately so the line-by-line correspondence
// to the Fortran source is preserved.
KRATOS_API(MPM_APPLICATION) void matmul(const double* a,
            const double* b,
            double* c,
            std::size_t l,
            std::size_t m,
...
KRATOS_API(MPM_APPLICATION) StrainInvariants inv_eps(const double* eps);
...
struct StressInvariants {
    ...
    // *negative* I2 here. Do not "correct" the sign without a matching
    // change in the Fortran reference.
    double I2;
...
// not a physically accurate Lode angle. Do not "fix" this without a
// matching change in the Fortran reference.
KRATOS_API(MPM_APPLICATION) StressInvariants inv_sig(const double* sig);

// =====================================================================
// Constitutive tangent: `get_tan`
// =====================================================================
//
// Computes the von Wolffersdorff (1996) sand-hypoplastic +
// Niemunis & Herle (1997) intergranular-strain tangent at the current
// step. Mirrors `get_tan_h` in `00_hypo_is_original.for` (.for:631-997)
// verbatim. This is the physics core of the kernel; it is intentionally
// a literal port so numerical equivalence with the reference
// implementation stays verifiable.
...
struct TangentResult {
    ...
    //     this path.
    //   - r_uc <= 0 defensive guard (deviation from the Fortran
    //     reference, no .for equivalent): fires AFTER L/N are computed,
...
//     fallback (see .for:766 and `integrate_step`). With I1 >= 0
//     this routine produces NaN via `pow(-I1/hs, en)`.
...
// Don't depend on either pattern; failure-path tests should assert
// only on `error` and, at most, on M and H.
//
// Deliberate deviation from Fortran (defense-in-depth alongside the
// check_parms validation):
...
//   - check_parms rejects the cross-parameter combination
...
//     a serialized blob, future test seams).
KRATOS_API(MPM_APPLICATION) TangentResult get_tan(const double* deps,
                      const double* sig,
                      const double* q,
                      const Parms16& parms,
                      int istrain);
...
//   with negative base (.for:833), and this routine propagates the
//   NaN into F_sig and F_q while reporting error == 0. The Fortran
//   architecture handles this via a `check_RKF_h` gate at the RKF
//   substep boundary (.for:1768-1831), translated here as `check_RKF`;
//   drivers that invoke it before each rhs evaluation (as
//   `rkf23_update` does) enforce this contract structurally.
...
KRATOS_API(MPM_APPLICATION) RhsResult get_F_sig_q(const double* deps,
                      const double* sig,
                      const double* q,
                      const Parms16& parms);
...
KRATOS_API(MPM_APPLICATION) YdotResult rhs(const double* y,
               const double* deps,
               const Parms16& parms);
...
// for the stress regimes exercised here (compressive, mildly
// anisotropic).
//
...
// this port drops that swap entirely and keeps the kernel consistent
...
KRATOS_API(MPM_APPLICATION) PrincipalStresses principal_stresses_3(const double* sig);
...
// check that fired, with class precedence:
...
//
// When an RKF driver invokes check_RKF before each rhs evaluation
// (as `rkf23_update` does), the get_F_sig_q precondition becomes
...
KRATOS_API(MPM_APPLICATION) CheckRkfResult check_RKF(const double* y, const Parms16& parms);
...
// it would expose callers to undefined values whenever the driver
// passes the test=3 sentinel `youngel = -100`. The behaviour here is
// "no elastic update", which is what the umat driver would have
// wanted in that branch.
...
KRATOS_API(MPM_APPLICATION) ElasticIncrementResult calc_elasti(const double* deps,
                                   double youngel,
                                   double nuel);
...
// rather than y_hat). Catching them at the source avoids relying on
// every caller to recheck.
...
KRATOS_API(MPM_APPLICATION) NormResResult norm_res(const double* y_til, const double* y_hat);
...
//            failure_class propagates through Rkf23Result so
//            integrate_step does not demote this to RkfReject.
...
// to route on retryability. `error` alone is Fortran-faithful (values
// 0/3/10 mirror the umat reference) but conflates a retryable RKF
// reject with a contract violation that looks identical (both report
// `error = 3`). The wrapping law must escalate contract violations to
// KRATOS_ERROR rather than cutback the outer step (a smaller dtime
// cannot fix corrupt input).
//
...
//
// `failure_class` is set precisely at EVERY error exit --
// `integrate_step`'s OWN entry, the initial `check_RKF` gate, AND
// `rkf23_update`'s mid-substep gates (including `check_RKF`
// corruption-class labels in k1/k2/k3/y_hat stages and
// `nres.nan_detected` for poisoned residuals). No rkf23-internal
// corruption is silently demoted to `RkfReject`. This is one more
// deviation from Fortran's int-only error model (.for:302-316
// conflates all RKF-side error == 3 unconditionally), consistent with
// the kernel's "explicit fail-closed on every detected corruption
// signal" policy.
enum class FailureClass {
    ...
    ContractViolation,   // error == 3, NOT retryable: input or derived
                         ...
                         //   residual). Callers must escalate, not retry.
...
KRATOS_API(MPM_APPLICATION) Rkf23Result rkf23_update(const double* y_init,
                         double dtsub_init,
                         double err_tol,
                         int    maxnint,
                         double DTmin,
...
// =====================================================================
//
// Pure-C++ analog of the Fortran `umat` driver: a step-level
...
// belong in the constitutive law that wraps this routine.
//
...
//       overflow / NaN / non-positive-void condition.
...
//          (DISTINCT from RkfReject; the wrapping law MUST escalate as
...
//          before calling get_tan_h (.for:1466-1475). Tangent
//          agreement with the reference implementation requires this
//          convention exactly.
...
//          guard or fb_temp1 guard); same y revert + D = 0; the
//          wrapping law maps this to KRATOS_ERROR.
...
// (`[11, 22, 33, 12, 13, 23]`) and the kernel-internal q. The wrapping
// constitutive law is responsible for the Kratos slot-4/5 swap.
//
...
//     RkfReject -- a caller that cuts back the outer dtime would
...
//     as a contract violation deadlocks the caller (it cuts back,
//     reads zero again, never makes progress).
//
// All scalar contract failures classify as
// `FailureClass::ContractViolation` -- the wrapping law MUST escalate
// as KRATOS_ERROR, NOT cutback. Sets r.error = 3 + r.dtsub_next = 0.
//
...
// error == 0 and used_elastic == true.

struct StepResult {
    double sig[6];          // updated EFFECTIVE Cauchy stress
                            // (tension-positive, Voigt-Abaqus). The
                            // wrapping law converts to total via
                            //   sig_total_next = sig_eff_next - pore_next * I_normal
                            // (normal slots only). The kernel never
                            // sees total stress or pore pressure.
    ...
    FailureClass failure_class;  // Caller routing signal; see enum docstring.
};

KRATOS_API(MPM_APPLICATION) StepResult integrate_step(const double* sig_n,
                          const double* q_n,
                          const double* deps_np1,
                          const Parms16& parms,
                          double dtime,
...
#endif // KRATOS_SAND_HYPOPLASTIC_KERNEL_H_INCLUDED
