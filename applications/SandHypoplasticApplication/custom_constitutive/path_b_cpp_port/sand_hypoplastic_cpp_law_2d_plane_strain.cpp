// Path B - SandHypoplasticCppLaw2DPlaneStrain (implementation).
...
// to 2D.
#include "sand_hypoplastic_cpp_law_2d_plane_strain.h"
...
constexpr std::array<std::size_t, 3> SandHypoplasticCppLaw2DPlaneStrain::k2Dto3D;

//********************************************************************
// Lifecycle / Clone
//********************************************************************

SandHypoplasticCppLaw2DPlaneStrain::SandHypoplasticCppLaw2DPlaneStrain()
    : ConstitutiveLaw(), m3DLaw() {}

SandHypoplasticCppLaw2DPlaneStrain::SandHypoplasticCppLaw2DPlaneStrain(const SandHypoplasticCppLaw2DPlaneStrain& rOther)
    : ConstitutiveLaw(rOther), m3DLaw(rOther.m3DLaw) {}

ConstitutiveLaw::Pointer SandHypoplasticCppLaw2DPlaneStrain::Clone() const
{
    return Kratos::make_shared<SandHypoplasticCppLaw2DPlaneStrain>(*this);
}
...
void SandHypoplasticCppLaw2DPlaneStrain::GetLawFeatures(Features& rFeatures)
{
...
void SandHypoplasticCppLaw2DPlaneStrain::CalculateMaterialResponseCauchy(Parameters& rValues)
{
    RunInner3D(rValues, /*is_finalize=*/false);
}

void SandHypoplasticCppLaw2DPlaneStrain::FinalizeMaterialResponseCauchy(Parameters& rValues)
{
    RunInner3D(rValues, /*is_finalize=*/true);
}

void SandHypoplasticCppLaw2DPlaneStrain::RunInner3D(Parameters& rValues_2D, bool is_finalize)
{
    ...
    KRATOS_ERROR_IF(F2D.size1() != 2 || F2D.size2() != 2)
        << "SandHypoplasticCppLaw2DPlaneStrain: expected 2x2 F from Kratos MPM 2D, got "
    ...
    KRATOS_ERROR_IF(strain2D.size() != kVoigt2D)
        << "SandHypoplasticCppLaw2DPlaneStrain: expected 3-component 2D Voigt strain, got "
        << strain2D.size();
    ...
    // constitutive law (the bridge ignores them), but pass-through is
    // free and makes the inner Parameters object structurally
    // equivalent.
    ...
    // Strain write-back. The 3D bridge writes Almansi-from-F-3D into
    ...
    // we padded in -- a no-op for the caller's state.
